...
struct StackingImage
{
	StackingImage() : width(0), height(0), dynamic_range(0) {}
	StackingImage(int w, int h, int num_pixels) : width(w), height(h), pixels(num_pixels), dynamic_range(0) {}
	int width;
	int height;
	...
	StackingImage LpFilter(LpFilterConfig const &config) const;
	Pwl CreateTonemap(GlobalTonemapConfig const &config) const;
	void Tonemap(StackingImage const &lp, StackingConfig const &config);
...
void StackingImage::Accumulate(uint8_t const *src, int stride)
{
	int16_t *dest = &P(0);
...
static void forward_pass(std::vector<double> &fwd_pixels, std::vector<double> &fwd_weight_sums, StackingImage const &in,
...
StackingImage StackingImage::LpFilter(LpFilterConfig const &config) const
{
	...
	StackingImage out(width, height, width * height);
...
Histogram StackingImage::CalculateHistogram() const
{
...
Pwl StackingImage::CreateTonemap(GlobalTonemapConfig const &config) const
{
...
void StackingImage::Tonemap(StackingImage const &lp, StackingConfig const &config)
{
...
void StackingImage::Extract(uint8_t *dest, int stride) const
{
...
void StackingImage::Scale(double factor)
{
...
class StackingStage : public PostProcessingStage
{
public:
	StackingStage(RPiCamApp *app) : PostProcessingStage(app) {}
...
private:
	...
	std::mutex mutex_;
	StackingImage acc_, lp_;
};

#define NAME "stacking"

char const *StackingStage::Name() const
{
	return NAME;
}

void StackingStage::Read(boost::property_tree::ptree const &params)
{
...
void StackingStage::AdjustConfig(std::string const &use_case, StreamConfiguration *config)
{
...
void StackingStage::Configure()
{
	stream_ = app_->StillStream(&info_);
	if (!stream_)
		return;
	if (stream_->configuration().pixelFormat != libcamera::formats::YUV420)
		throw std::runtime_error("StackingStage: only supports YUV420");

	// No stacking for  shutter times bellow 11.7s
	Options const *options = dynamic_cast<Options *>(app_->GetOptions());
	if (options->shutter.get() <= 11767556)
		config_.num_frames = 1;

	// Allocate and initialise the big accumulator image.
	frame_num_ = 0;
	acc_ = StackingImage(info_.width, info_.height, info_.width * info_.height * 3 / 2);
	acc_.Clear();
	lp_ = StackingImage(info_.width, info_.height, info_.width * info_.height);
}

bool StackingStage::Process(CompletedRequestPtr &completed_request)
{
	if (!stream_)
	...
	// No stacking, just quit
	if (config_.num_frames == 1)
	{
		return false;
	}
	...
	// Do HDR processing.
	LOG(1, "Doing stacking processing...");
